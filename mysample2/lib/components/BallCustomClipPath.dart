...
class BallCustomClipPath extends CustomClipper<Path> {
  ...
  BallCustomClipPath(this.ballPositionX, this.ballPositionY, this.isPermitReading);
  ...
  Path getClip(Size size) {
    ...
    path.addOval(Rect.fromCircle(center: Offset(validWidth*0.5+ballDrawingMargin, validHeight*0.5+ballDrawingMargin), radius: validWidth*permitTilt));
    double positionX=ballPositionX+ballDrawingMargin;
    double positionY=ballPositionY+ballDrawingMargin;
    path.addOval(Rect.fromCircle(center:Offset(positionX,positionY),radius: ballRadius));
    path.close();
